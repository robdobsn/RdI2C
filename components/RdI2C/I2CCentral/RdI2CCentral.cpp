...
static const char* MODULE_PREFIX = "RdI2CCentral";
...
bool RdI2CCentral::init(uint8_t i2cPort, uint16_t pinSDA, uint16_t pinSCL, uint32_t busFrequency,
...
bool RdI2CCentral::isOperatingOk()
{
...
RdI2CCentralIF::AccessResultCode RdI2CCentral::access(uint16_t address, uint8_t* pWriteBuf, uint32_t numToWrite,
...
bool RdI2CCentral::ensureI2CReady()
{
    // Check if busy
    if (isBusy())
    {
...
void RdI2CCentral::prepareI2CAccess()
{
    ...
    // Max timeout
    // 0xfffff == 13ms
    // 0x3e80 is used by ESP IDF == 200us
    I2C_DEVICE.timeout.tout = 0x8000;
...
void RdI2CCentral::reinitI2CModule()
{
    // Clear interrupts
    I2C_DEVICE.int_ena.val = 0;
    I2C_DEVICE.int_clr.val = UINT32_MAX;
...
bool RdI2CCentral::setBusFrequency(uint32_t busFreq)
{
...
uint32_t RdI2CCentral::getApbFrequency()
{
...
void RdI2CCentral::setI2CCommand(uint32_t cmdIdx, uint8_t op_code, uint8_t byte_num, bool ack_val, bool ack_exp, bool ack_en)
{
    // Check valid
...
bool RdI2CCentral::initInterrupts()
{
...
void RdI2CCentral::initBusFiltering()
{
...
bool RdI2CCentral::checkI2CLinesOk()
{
...
void IRAM_ATTR RdI2CCentral::i2cISRStatic(void* arg)
{
    // Get object
    RdI2CCentral* pArg = (RdI2CCentral*)arg;
    if (pArg)
        pArg->i2cISR();
}

void IRAM_ATTR RdI2CCentral::i2cISR()
{
    ...
    _i2cStats.update(intStatus & I2C_TRANS_START_INT_ST,
            ...
            intStatus & I2C_TX_SEND_EMPTY_INT_ST
    );
...
uint32_t IRAM_ATTR RdI2CCentral::fillTxFifo()
{
...
uint32_t IRAM_ATTR RdI2CCentral::emptyRxFifo()
{
    // Check valid
    if (!_readBufStartPtr)
...
String RdI2CCentral::debugMainStatusStr(const char* prefix, uint32_t statusFlags)
{
...
String RdI2CCentral::debugFIFOStatusStr(const char* prefix, uint32_t statusFlags)
{
    char outStr[60];
...
String RdI2CCentral::debugINTFlagStr(const char* prefix, uint32_t statusFlags)
{
    const char* intStrs[] = {
        "RxFIFOFull",
        "TxFIFOEmpty",
...
void RdI2CCentral::debugShowStatus(const char* prefix, uint32_t addr)
{
